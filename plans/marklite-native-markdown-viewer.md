...
- [x] **Logo** (user request 2026-08-23): design a cool, recognizable MarkLite mark — SVG master in `assets/` (works at 16 px and 256 px, legible in dark and light contexts; propose 2–3 candidates, user picks). Derive multi-res `MarkLite.ico` (16/24/32/48/64/128/256). _(two rounds: user picked the "plan signature" direction — the app's own accent task checkbox + plan lines — then the feather variant, "lite" as lightweight. 16/24/32 ico slots carry a simplified checkbox-only master `MarkLite-small.svg` because lines+feather blur below 48 px; ico assembled from both SVGs with PNG-compressed slots)_
